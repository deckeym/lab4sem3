...
class Semaphore { // реализация semaphore
    ...
    Semaphore(int initialCount) : iCount(initialCount) {
        sem_init(&sem, 0, initialCount);    // POSIX-функция для инициализации объекта семафора
    }
    void acquire() { // захватывает ресурс
        sem_wait(&sem); // sem--
    }

    void release() { // освобождает ресурс
        sem_post(&sem); // sem++
    }

    private:
    sem_t sem;  // Объект POSIX-семафора
...
class Barrier { // реализация Barrier 
    ...
    void wait() { // ожидает, пока все потоки не достигнут барьера
        ...
        cout << "Thread " << this_thread::get_id() << " reached the barrier.\n"; // Логируем подход потока к барьеру
        if (--initialCount == 0) { // если счётчик 0 - все потоки достигли барьера
            ...
            cout << "All threads reached the barrier. Barrier unlocked.\n"; // Логируем разблокировку барьера
    ...
    // счётчик поколений для уведомления потоков, что барьер потоков достигнут
};
...
void threadSpinLock(char& symbol, atomic_flag& spinLock, vector<char>& allSymbols) { // spinLock
    for (int i = 0; i < N; i++) {
        randomSymbols(symbol);
        while (spinLock.test_and_set(memory_order_acquire)) {} // устанавливаем флаг, поток входит в режим ожидания, пока флаг не будет сброшен
        allSymbols.push_back(symbol);
